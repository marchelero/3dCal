...
void main() {
  group('EntitlementNotifier.purchase()', () {
    ...
    test('PaymentSuccess → activate: state Pro + repo.save + cache true',
        () async {
    ...
    test('PaymentError → sin cambios (state sigue Free, no cache write)',
        () async {
      await setupContainer();
      await container.read(entitlementNotifierProvider.future);
  ...
  group('EntitlementNotifier.restore()', () {
    ...
    test('RestoreActive → activate: state Pro + repo.save + cache true',
        () async {
      await setupContainer();
      await container.read(entitlementNotifierProvider.future);
    ...
    test('RestoreEmpty → deactivate: state Free + repo.clear + cache cleared',
        () async {
      await setupContainer();
      await container.read(entitlementNotifierProvider.future);
    ...
    test('RestoreEmpty desde Pro (cache Pro + DB active) → downgrade a Free',
        () async {
  ...
  group('EntitlementNotifier boot — stale vs fresh cache', () {
    ...
    test('cache Pro stale (validatedAt > 7 dias) → boot dispara restore()',
        () async {
      final stale = DateTime.now().toUtc().subtract(const Duration(days: 8));
      final container = await setupContainer(spInitial: <String, Object>{
        kIsProKey: true,
        kEntitlementSourceKey: kSourceLifetimePurchase,
    ...
    test('cache Pro fresh (validatedAt < 7 dias) → NO dispara restore()',
        () async {
    ...
    test('cache Pro + validatedAt=null (stale por default) → dispara restore',
        () async {
      final container = await setupContainer(spInitial: <String, Object>{
        kIsProKey: true,
        kEntitlementSourceKey: kSourceLifetimePurchase,
    ...
    test('cache Free (o vacio) → NO dispara restore() en boot', () async {
      ...
      expect(paymentService.restoreCalls, 0,
          reason: 'Cache Free no debe disparar restore() — no hay nada '
              'que validar.');
    });
  });
}
