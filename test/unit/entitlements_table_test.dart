...
void main() {
  ...
  group('Entitlements table — schema', () {
    test(
      'esta registrada en @DriftDatabase con tableName "entitlements"',
      () async {
        ...
        expect(
          ...
          isNotEmpty,
          reason:
              'entitlements debe estar en @DriftDatabase(tables:). '
    ...
    test('existe en SQLite tras onCreate (pragma_table_info)', () async {
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'La tabla `entitlements` debe existir en la DB tras '
            'onCreate. Si fallo, m.createAll() no la esta creando — '
            'verificar que la clase esta en @DriftDatabase(tables:).',
      );
    });

    test('tiene todas las columnas esperadas con tipos SQL y nullability '
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'pragma_table_info(\'entitlements\') devolvio vacio. '
      ...
      expect(
        byName['source']!.read<String>('type'),
        'TEXT',
        reason: 'source debe ser TEXT.',
      );
      expect(
        byName['source']!.read<int>('isNotNull'),
        1,
      ...
      expect(
        ...
        contains('product_id'),
        reason:
            'Falta columna product_id (NOT NULL). Drift genera '
            'snake_case desde el field Dart `productId`.',
      );
      expect(
        byName['product_id']!.read<String>('type'),
        'TEXT',
        reason: 'product_id debe ser TEXT.',
      );
      expect(
        byName['product_id']!.read<int>('isNotNull'),
        1,
      ...
      expect(
        ...
        reason:
            'Falta columna purchased_at (NOT NULL). Drift genera '
      ...
      expect(
        ...
        0,
        reason:
            'validated_at debe ser NULLABLE '
      ...
      expect(
        byName.keys,
        contains('receipt_data'),
        reason: 'Falta columna receipt_data (nullable).',
      );
      expect(
        byName['receipt_data']!.read<String>('type'),
        'TEXT',
        reason: 'receipt_data debe ser TEXT.',
      );
      expect(
        byName['receipt_data']!.read<int>('isNotNull'),
        0,
      ...
      expect(
        byName['is_active']!.read<int>('isNotNull'),
        1,
