...
void main() {
  ...
  test('happy path: purchase() success → DB row + SP cache true + isPro=true',
      () async {
  ...
  test('cancel path: purchase() cancel → DB vacia + SP sin cache + isPro=false',
      () async {
    final container = await setupContainer();
    await container.read(entitlementNotifierProvider.future);
  ...
  test('error path: purchase() error → DB vacia + sin cache + isPro=false',
      () async {
    final container = await setupContainer();
    await container.read(entitlementNotifierProvider.future);
  ...
  test('restore path: restore() active → DB row + SP cache + isPro=true',
      () async {
    final container = await setupContainer();
    await container.read(entitlementNotifierProvider.future);
  ...
  test('restore empty path: restore() empty → DB cleared + SP cleared + isPro=false',
      () async {
    final container = await setupContainer();
    await container.read(entitlementNotifierProvider.future);
    ...
    final active = await (db.select(db.entitlements)
